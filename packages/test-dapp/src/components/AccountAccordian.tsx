import { ConnectorType, IDENTITY_PROVIDERS, WalletConnectSessionStruct } from "@almight-sdk/connector";
import { Providers } from "@almight-sdk/utils";
import React from "react";
import { Accordion, ListGroup } from "react-bootstrap"
import { IdentityProviderData } from "../types"
import SessionHolder from "./SessionHolder"

interface AccountAccordianProps {
    data: IdentityProviderData
}

const Images: Record<string, string> = {
    [Providers.MetaMask]: "https://upload.wikimedia.org/wikipedia/commons/thumb/3/36/MetaMask_Fox.svg/1024px-MetaMask_Fox.svg.png",
    [Providers.KardiaChain]: "data:image/jpeg;base64,",
    "1": "https://upload.wikimedia.org/wikipedia/commons/thumb/0/01/Ethereum_logo_translucent.svg/800px-Ethereum_logo_translucent.svg.png",
    "56": "https://public.bnbstatic.com/20190405/eb2349c3-b2f8-4a93-a286-8f86a62ea9d8.png",
    [ConnectorType.WalletConnector]: "https://seeklogo.com/images/W/walletconnect-logo-EE83B50C97-seeklogo.com.png"
}


const avatar = "https://i.stack.imgur.com/frlIf.png"

const AccountAccordian: React.FC<AccountAccordianProps> = (props: AccountAccordianProps) => {

    function addressCrunch(address: string): string {
        return `${address.slice(0, 7)}....${address.slice(address.length - 6)}`
    }

    function onSessionClick(session: any){
        console.log(session)
    }

    const listicls = props.data.sessions.map((session, index) => {
        let data: any = {
            footer: Images[session.chainId.toString()],
            session: session,
            onClick: onSessionClick,
            key: `${props.data.uid}_${index}`
        }
        if((session as WalletConnectSessionStruct).bridge !== undefined ){
            data["header"] = Images[ConnectorType.WalletConnector];
            data["text"] = "Wallet Connect"
        }else{
            data["header"] = Images[props.data.provider]
            data["text"] = IDENTITY_PROVIDERS[props.data.provider].identityProviderName
        }
        return <SessionHolder {...data} />
    })

    

    return (
        <Accordion.Item eventKey="test">
            <Accordion.Header as="div">
                <div className="w-full h-auto flex flex-row">
                    <img src={avatar} className="w-12 rounded-md" placeholder="Avatar" alt="avatar" />
                    <p className="mx-6 my-0 bg-orange-100 px-4 rounded-md py-2 self-center font-bold text-lg">{addressCrunch(props.data.uid)}</p>
                </div>
            </Accordion.Header>
            <Accordion.Body>
                <ListGroup>
                   {listicls}
                </ListGroup>
            </Accordion.Body>
        </Accordion.Item>
    )
}

export default AccountAccordian;